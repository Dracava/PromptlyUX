                ...
                figma.ui.once('message', (parsedData) => __awaiter(this, void 0, void 0, function* () {
                    if (parsedData.type === 'parsed-html-result') {
                        ...
                        for (let i = 0; i < elements.length; i++) {
                            ...
                            const isColorSection = elementText.toLowerCase().includes('color palette') ||
                            ...
                            if (isPersona) {
                                ...
                                const isPersonaHeader = element.type === 'h1' ||
                                    element.type === 'h2' ||
                                    element.type === 'h3' ||
                                    element.type === 'h4' ||
                                    element.type === 'h5' ||
                                ...
                                const isPersonaSubheading = !isPersonaHeader &&
                            ...
                            if (element.type === 'bullet-list') {
                                ...
                                if (currentTextGroupType === null || currentTextGroupType === 'bullet-list') {
                                    ...
                                    const isFontExample = yield processFontExample(groupedText, textNode);
                                    // Use mentioned font if available and not a font example
                                    if (!isFontExample && groupFontName) {
                                        try {
                                            yield figma.loadFontAsync({ family: groupFontName, style: "Regular" });
                            ...
                            // Handle numbered lists - create separate text boxes for each item
                                ...
                                switch (element.type) {
                                    case 'strong':
                                        if (mentionedFont) {
                                            try {
                                                yield figma.loadFontAsync({ family: mentionedFont, style: "Bold" });
                                    ...
                                    case 'em':
                                        if (mentionedFont) {
                                            try {
                                                yield figma.loadFontAsync({ family: mentionedFont, style: "Italic" });
